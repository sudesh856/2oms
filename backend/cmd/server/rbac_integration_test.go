...
func TestEveryRegisteredRouteEnforcesRBAC(t *testing.T) {
	...
	testCases := []rbacRouteCase{
		...
		{name: "update order", method: http.MethodPatch, path: "/api/orders/not-a-uuid", body: `{}`, allowed: allRoles(), wantStatus: http.StatusBadRequest},
		...
		{name: "list couriers", method: http.MethodGet, path: "/api/couriers", allowed: allRoles(), wantStatus: http.StatusOK},
		...
		{name: "list locations", method: http.MethodGet, path: "/api/couriers/not-a-uuid/locations", allowed: allRoles(), wantStatus: http.StatusBadRequest},
